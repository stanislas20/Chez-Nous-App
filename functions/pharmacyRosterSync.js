...
// four individually).
const REGIONS = [
  { postingRegion: 'Littoral', departments: ['Littoral'], categoryUrl: 'https://onpb.bj/category/tour-de-garde/littoral/' },
  { postingRegion: 'Atlantique', departments: ['Atlantique'], categoryUrl: 'https://onpb.bj/category/tour-de-garde/atlantique/' },
  {
  ...
  },
  {
    ...
    categoryUrl: 'https://onpb.bj/category/tour-de-garde/zou-collines-mono-couffo/',
  },
  {
    postingRegion: 'Atacora-Donga-Borgou-Alibori',
    departments: ['Atacora', 'Donga', 'Borgou', 'Alibori'],
    categoryUrl: 'https://onpb.bj/category/tour-de-garde/atacora-donga-borgou-alibori/',
  },
];
...
const DATE_RANGE_RE = /(\d{1,2})\s*(janvier|f[ée]vrier|mars|avril|mai|juin|juillet|ao[uû]t|septembre|octobre|novembre|d[ée]cembre)\s*(\d{4})/gi;
...
async function fetchLatestPost(categoryUrl) {
  ...
  if (!response.ok) throw new Error(`Category page fetch failed: ${response.status}`);
  const html = await response.text();

  const linkMatch = /<a[^>]+href="(https:\/\/onpb\.bj\/programme-de-garde[^"]*)"[^>]*>/i.exec(html);
...
async function fetchPostImages(postUrl) {
  ...
  const seen = new Set();
  const images = [];
  const imgRe = /https:\/\/onpb\.bj\/wp-content\/uploads\/\d{4}\/\d{2}\/[^\s"']*?\.(?:jpe?g|png|webp)/gi;
  for (const match of html.matchAll(imgRe)) {
    ...
    if (/logo-onpb|ban2\.jpg|elementor\/thumbs|ultimatemember/i.test(url)) continue;
...
const BENIN_DEPARTMENTS_BY_LOWER = new Map(BENIN_DEPARTMENTS.map((d) => [d.toLowerCase(), d]));
...
const CANONICAL_CITIES = [
  'Cotonou', 'Porto-Novo', 'Parakou', 'Abomey-Calavi', 'Bohicon', 'Ouidah', 'Djougou', 'Natitingou',
  'Allada', 'Toffo', 'Torri-Bossito', 'Zè', 'Kpomassè', 'Sèmè-Kpodji', 'Akpro-Missérété', 'Adjarra',
  'Pobè', 'Abomey', 'Agbangnizoun', 'Aplahoué', 'Bantè', 'Bopa', 'Comè', 'Covè', 'Dassa-Zoumè',
  'Djakotomey', 'Djidja', 'Dogbo', 'Glazoué', 'Grand-Popo', 'Houéyogbé', 'Klouékanmè', 'Lalo',
  'Lokossa', 'Ouèssè', 'Ouinhi', 'Savalou', 'Savè', 'Toviklin', 'Za-Kpota', 'Zogbodomey', 'Kérou',
  'Tanguiéta', 'Bassila', 'Copargo', 'Ouaké', 'Banikoara', 'Kandi', 'Malanville', 'Bembéréké',
  "N'Dali", 'Nikki', 'Tchaourou', 'Avrankou', 'Ifangni', 'Dangbo', 'Adjohoun', 'Sakété', 'Ikpinlé',
  'Adja-Ouèrè', 'Kétou',
];
...
const CANONICAL_CITIES_BY_FOLD = new Map(CANONICAL_CITIES.map((c) => [foldCityKey(c), c]));
...
function normalizeEntries(entries, region) {
  ...
  const normalized = (entries || []).map((entry) => {
    ...
    if (region.departments.length === 1) {
      ...
      department = BENIN_DEPARTMENTS_BY_LOWER.get((entry.department || '').trim().toLowerCase()) ?? entry.department;
    }
    ...
    const results = numbers.map((n) => {
      ...
      if (n.kind === 'old-format' && !hasModernSibling) return { ...n, ok: true, value: `01${n.digits}` };
    ...
    for (const r of ambiguous) {
      ambiguousPhones.push(`${entry.name}: "${r.raw}" (${r.digits.length} digits after stripping)`);
    }
    ...
    let name = entry.name;
    let city = entry.city;
    if (region.postingRegion === 'Littoral' && entry.city && entry.city !== 'Cotonou') {
      const zone =
        entry.zone && entry.zone.trim() && entry.zone.trim().toLowerCase() !== entry.city.trim().toLowerCase()
          ? entry.zone.trim()
          : null;
      name = zone ? `${entry.name} (${entry.city}, ${zone})` : `${entry.name} (${entry.city})`;
      city = 'Cotonou';
    } else {
...
async function getReviewerPushToken() {
  ...
  if (!reviewerUid) {
    logger.info('No pharmacyRosterReviewer configured — skipping push notification.');
    return null;
  }
  ...
  if (!pushToken) {
    logger.info(`Reviewer ${reviewerUid} has no pushToken registered — skipping push notification.`);
    return null;
  }
  return pushToken;
}
...
async function sendReviewerPush(title, body, data) {
  ...
  try {
    await admin.messaging().send({ token: pushToken, notification: { title, body }, data });
  } catch (error) {
...
async function syncPharmacyRosters(apiKey) {
  ...
  for (const region of REGIONS) {
    try {
      ...
      if (stateSnap.exists && stateSnap.data().lastProcessedPostUrl === latest.postUrl) {
      ...
      if (imageUrls.length === 0) {
        logger.warn(`${region.postingRegion}: post found but no roster images detected — ${latest.postUrl}`);
        continue;
      }

      const transcription = await transcribeRosterImages(imageUrls, apiKey);
      const { entries: normalizedEntries, ambiguousPhones, unresolvedCities } = normalizeEntries(
      ...
      const dateMismatch =
        weekEndDate && titleWeekEndDate && weekEndDate.getTime() !== titleWeekEndDate.getTime();
      ...
      const expectedDepartments = new Set(region.departments.map((d) => d.toLowerCase()));
      const unexpectedDepartments = [
        ...new Set(normalizedEntries.map((e) => (e.department || '').trim()).filter(Boolean)),
      ...
      await db.collection('pharmacyRosterDrafts').add({
        ...
        dutyUntil: weekEndDate ? admin.firestore.Timestamp.fromDate(weekEndDate) : null,
      ...
      newDraftCount += 1;
      logger.info(`${region.postingRegion}: drafted ${transcription.entries?.length ?? 0} entries from ${latest.postUrl}`);
    } catch (error) {
...
exports.syncPharmacyRosters = onSchedule(
  { schedule: '0 8 * * *', timeZone: 'Africa/Porto-Novo', secrets: [anthropicApiKey], timeoutSeconds: 300 },
  async () => {
