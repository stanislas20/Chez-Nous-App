...
function formatPrice(listing) {
  if (listing.categoryKey === 'pharmacies' || listing.categoryKey === 'jobs') return '—';
...
async function listPending() {
  ...
  for (const doc of docs) {
    ...
    const media = (listing.media ?? []).map((item) => item.mediaUrl).filter(Boolean);
    console.log(`\n${doc.id}`);
    console.log(`  Title:    ${listing.titleFr || listing.titleEn || '(untitled)'}`);
    ...
    console.log(`  Photos:   ${media.length ? media.join('\n            ') : 'none'}`);
    console.log(`  Posted:   ${listing.createdAt?.toDate?.() ?? 'n/a'}`);
    console.log(`  Text:     ${(listing.descriptionFr || listing.descriptionEn || '').slice(0, 200)}`);
  }
...
async function setStatus(listingId, status, note) {
  ...
  if (status === 'rejected' && !note) {
    console.log('  Note: no reason given — the seller will see a generic message.');
  }
}
...
async function main() {
  ...
  if (!listingId || listingId.startsWith('--')) {
    console.error('Usage: node scripts/moderateListing.js <listingId> [--reject "reason"]');
