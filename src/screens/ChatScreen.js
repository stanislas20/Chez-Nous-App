...
import { Alert, Animated, FlatList, PanResponder, Platform, Pressable } from 'react-native';
...
function VoiceMessageBubble({ uri, mine, knownDuration }) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        if (d > 0) p.seekTo(Math.max(0, Math.min(1, evt.nativeEvent.locationX / SCRUB_TRACK_WIDTH)) * d);
      },
  ...
  const displaySeconds =
    status.playing || status.currentTime > 0 ? status.currentTime : effectiveDuration;
  ...
  return (
    <VoiceContainer>
      ...
      <VoiceTrackColumn>
        ...
        <VoiceDuration mine={mine}>{formatDuration(displaySeconds)}</VoiceDuration>
      </VoiceTrackColumn>
    </VoiceContainer>
  );
}
...
export function ChatScreen({ route, navigation }) {
  ...
  const blockedByOther = Boolean(otherUid && conversation?.blockedBy?.[otherUid]);
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      title: listingTitle,
      headerRight: () => (
        <Pressable onPress={handleOpenMenu} hitSlop={12} style={{ marginRight: spacing.md }}>
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(firestore, 'conversations', conversationId), (snapshot) => {
  ...
  const updateLastMessage = async ({ messageType, preview }) => {
    const otherParticipant = conversation.participantIds.find((id) => id !== user.uid);
  ...
  const handleDeleteMessage = (message) => {
    ...
    Alert.alert(t('chatDeleteMessageTitle'), t('chatDeleteMessageConfirm'), [
      { text: t('cancel'), style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () => {
          deleteDoc(doc(firestore, 'conversations', conversationId, 'messages', message.id)).catch(() => {});
        },
      },
    ]);
  };
  ...
  const handleTakePhoto = async () => {
    ...
    if (!permission.granted) {
      Alert.alert(t('errorChatFailedTitle'), t('errorGeneric'));
      return;
    }
  ...
  const handleSendRecording = async () => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        // Storage rules require a matching contentType — the blob's own
        // `type` from a local file:// URI is unreliable, so set it explicitly.
        const uploadTask = uploadBytesResumable(storageRef, blob, { contentType: 'audio/m4a' });
        uploadTask.on('state_changed', null, reject, resolve);
      });
  ...
  return (
    <Flex behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      <Container edges={['left', 'right', 'bottom']}>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <BubbleRow mine={isMine}>
                ...
                >
                  {item.imageUrl ? (
                    <MessageImage source={{ uri: item.imageUrl }} resizeMode="cover" />
                  ) : item.audioUrl ? (
                    <VoiceMessageBubble uri={item.audioUrl} mine={isMine} knownDuration={item.audioDuration} />
                  ) : (
        ...
        {isBlocked ? (
          <BlockedBanner>
            <BlockedBannerText>
              {iBlocked ? t('chatYouBlockedBanner') : t('chatBlockedByOtherBanner')}
            </BlockedBannerText>
            {iBlocked ? (
          ...
          <InputRow>
            {recordedClip ? (
              <>
                <IconButton onPress={handleDiscardRecording} disabled={isUploading} hitSlop={8}>
                  <Ionicons name="trash-outline" size={20} color={colors.textMuted} />
                </IconButton>
                <PreviewPlayerContainer>
                  <VoiceMessageBubble uri={recordedClip.uri} mine={false} knownDuration={recordedClip.duration} />
                </PreviewPlayerContainer>
                <SendButton onPress={handleSendRecording} disabled={isUploading}>
              ...
              <>
                <IconButton onPress={handleAttachImage} disabled={isUploading} hitSlop={8}>
                ...
                {text.trim() ? (
                  ...
                  <SendButton onPress={handleStartRecording} disabled={isUploading}>
                    <Ionicons name="mic-outline" size={18} color={colors.textInverse} />
                  </SendButton>
                )}
              </>
            )}
